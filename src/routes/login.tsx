...
function Login() {
  ...
  function handlePasswordSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (pw !== "CareGo2026!") { setError("Incorrect password. Demo password is: CareGo2026!"); return; }
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate({ to: DEMO_USERS[email.toLowerCase().trim()].destination }); }, 600);
  }
      ...
      {step === "register" && (
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label htmlFor="fname">First name</Label><Input id="fname" required autoFocus className="mt-1.5" placeholder="e.g. Sarah" /></div>
            <div><Label htmlFor="lname">Last name</Label><Input id="lname" required className="mt-1.5" placeholder="e.g. Whitfield" /></div>
          </div>
          <div><Label htmlFor="newpw">Choose a password</Label><PasswordInput id="newpw" placeholder="At least 12 characters" /></div>
