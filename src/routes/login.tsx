...
const ROLES: { role: Role; label: string; description: string; icon: React.ReactNode; destination: string }[] = [
  {
    role: "family",
    label: "Family / Customer",
    description: "Find and book care services",
    icon: <Heart className="h-5 w-5" />,
    destination: "/app/family",
  },
  {
    role: "provider",
    label: "Care Provider",
    description: "Manage shifts and schedule",
    icon: <Briefcase className="h-5 w-5" />,
    destination: "/app/provider",
  },
  {
    role: "business",
    label: "Business / Care Home",
    description: "Workforce and capacity management",
    icon: <Building2 className="h-5 w-5" />,
    destination: "/app/business",
  },
  {
    role: "admin",
...
function Login() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true);
    // Simulate auth then navigate to role-specific dashboard
    setTimeout(() => {
      setLoading(false);
      navigate({ to: selectedRoleData.destination });
    }, 600);
  }
